...
export class WuKongIMService {
  ...
  private _numericChannelType: number = 251
  ...
  setTarget(channelId: string, channelType: number) {
    if (!this._cfg) return
    this._cfg.target = channelId
    this._cfg.channelType = channelType === 1 ? 'person' : 'group'
    this._numericChannelType = channelType
  }

  async sendText(text: string, opts?: { to?: string; channelType?: number; clientMsgNo?: string; header?: any }) {
    if (!this._im || !this._cfg) throw new Error('WuKongIMService not ready')
    const to = opts?.to ?? this._cfg.target
    const ct = opts?.channelType ?? this._numericChannelType
    const payload = { type: 1, content: text }
    return this._im.send(to, ct, payload, { clientMsgNo: opts?.clientMsgNo, header: opts?.header })
  }

  async sendPayload(payload: any, opts?: { to?: string; channelType?: number; clientMsgNo?: string; header?: any }) {
    if (!this._im || !this._cfg) throw new Error('WuKongIMService not ready')
    const to = opts?.to ?? this._cfg.target
    const ct = opts?.channelType ?? this._numericChannelType
    return this._im.send(to, ct, payload, { clientMsgNo: opts?.clientMsgNo, header: opts?.header })
